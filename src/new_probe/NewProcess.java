...
public class NewProcess {
    ...
    NewProcess(String originalLine) {
        this.mainLine = originalLine;
        setResult();
    }

    String getMainResult() {
        return mainResult;
    }

    private void setResult() {
        this.mainResult = refact(mainLine);
    }
    ...
    private String algorithm(String[] array) {
        ...
        if (opened.contains("^")) { //шукаємо степені. навіть якщо їх там кілька - ми шукаємо в методі їх по одному
            ...
            Decouple decouple = new Decouple(opened);
            originalLine = originalLine.replace(opened, decouple.getResult());
